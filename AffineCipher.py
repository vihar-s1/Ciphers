...
def encrypt(text: str, key_a: int, key_b: int) -> str | None:
    '''
    - Encrypts the given plain text using key (key_a, key_b) if and only if key_a is invertible
    - Returns cipher text if successfully encrypted else returns None
    '''
...
def decrypt(cipher: str, key_a: int, key_b: int) -> str | None:
    '''
    - Decrypts the given cipher text using key (key_a, key_b) if and only if key_a is invertible
    - Returns the plain text if successfully decryted else returns None
    '''
    if __inverse.get(key_a, None) is None:
